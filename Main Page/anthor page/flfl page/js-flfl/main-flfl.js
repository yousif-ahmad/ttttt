...
function countDownTime() {
  ...
  let downSecond = setInterval(function () {
    ...
    arr.push(light);
    if (arr[arr.length - 1] === 1200) {
      light = arr[arr.length - 2];
    }
    --light;
    let minLight = (light * 100) / 1200;
    circleLight.style.backgroundImage = `linear-gradient(to right, #eac31c ${minLight}%, #1e2229 0%)`;
    ...
    if (
      ...
      circleLight.style.backgroundImage = `linear-gradient(to right, #eac31c ${0}%, #1e2229 0%)`;
